...
class UsersForm(SectionForm):
    ...
    def build_form(
        ...
        return FormSpec(
            title="form_users_title",
            subtitle="form_users_subtitle",
            fields=[
                FieldSpec(
                    key="credentials_file",
                    label="Credentials file",
                    i18n_key="form_users_credentials_file_label",
                    type="text",
                ...
                FieldSpec(
                    ...
                    required=False,
                    help="form_users_root_password_help",
                ),
                FieldSpec(
                    key="root_password_confirm",
                    label="Confirm root password",
                    i18n_key="form_users_root_password_confirm_label",
                    type="password",
                    default=draft_root_password_confirm,
                    required=False,
                    help="form_users_root_password_confirm_help",
                ),
                FieldSpec(
                    key="accounts_editor",
                    label="User accounts",
                    i18n_key="form_users_accounts_label",
                    type="subsection",
                    default=f"{len(extra_accounts)} configured",
                    required=False,
                    help="form_users_accounts_help",
                ),
            ],
        )
    ...
    def _account_form(account: UserAccountConfig, existing: bool) -> FormSpec:
        ...
        return FormSpec(
            title="form_users_account_title",
            subtitle=None,
            fields=[
                FieldSpec(key="name", label="Username", i18n_key="form_users_account_name_label",
                          type="text", default=account.name,
                          help="form_users_account_name_help"),
                FieldSpec(key="password", label="Password", i18n_key="form_users_account_password_label",
                          type="password", default=None, required=False,
                          help="form_users_account_password_help"),
                FieldSpec(key="password_confirm", label="Confirm password",
                          i18n_key="form_users_account_password_confirm_label",
                          type="password", default=None, required=False,
                          help="form_users_account_password_confirm_help"),
                FieldSpec(key="groups", label="Groups", i18n_key="form_users_account_groups_label",
                          type="list", default=list(account.groups) if account.groups else None,
                          required=False, help="form_users_account_groups_help"),
                FieldSpec(key="shell", label="Shell", i18n_key="form_users_account_shell_label",
                          type="text", default=account.shell, required=False,
                          help="form_users_account_shell_help"),
                FieldSpec(key="comment", label="Comment", i18n_key="form_users_account_comment_label",
                          type="text", default=account.comment, required=False),
                FieldSpec(key="create_home", label="Create home",
                          i18n_key="form_users_account_create_home_label",
                          type="bool", default=True if account.create_home is None else account.create_home,
                          required=False),
                FieldSpec(key="ssh_authorized_keys", label="SSH authorized keys",
                          i18n_key="form_users_account_ssh_keys_label",
                          type="list", default=list(account.ssh_authorized_keys) if account.ssh_authorized_keys else None,
                          required=False, help="form_users_account_ssh_keys_help"),
            ],
            actions=actions,
        )
